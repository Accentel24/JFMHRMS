...
if (isset($_REQUEST['submit'])) {
	...
	$menu = $_REQUEST['menu'];
	$result = mysqli_query($link, "select * from hrms_admin_login where empname='$empname'") or die(mysqli_error($link));
	...
	if ($rows > 0) {
		...
		$query1 = "update  hrms_admin_login set empname='$uname',pwd='$pwd' where empname='$ename'";
	} else {
		
		$query1 = "insert into hrms_admin_login(user,pwd,empname) values('$userid','$password','$empname1')";
	}
	...
	$sql = mysqli_query($link, "delete from hrms_hr_user where emp_id='$empname1'");


	for ($j = 0; $j < count($menu); $j++) {
		$menuname = $menu[$j];

		echo $t = "insert into hrms_hr_user(emp_id, menus, currentdate,auth_by) values('$empname1','$menuname',now(),'$uname')";
